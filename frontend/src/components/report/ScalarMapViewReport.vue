...
const total_score = computed(() => props.node?.total_score_report?? "-") //total score computation
const weight = computed(() => node.value?.user_weight_report ?? "-");
const justification = computed(() =>
  node.value?.user_justification_report ?? "No justification provided."
);

/*
const interpretation = computed(() => {
  return "Interpretation to be inserted.";
});
*/

const metricDescription = computed(() =>
  node.value?.metric_description_report || "Description not available."
);
  ...
  <div class="report-page-content">
    <div class="page-inner">
      <header class="title-block">
        <h1 class="page-title">{{ prettifyLabel(title) }}</h1>

        <div v-if="sensitiveFeature" class="feature-line">
          Sensitive feature: {{ prettifyLabel(sensitiveFeature) }}
        </div>
